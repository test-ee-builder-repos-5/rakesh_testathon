...
```bash
ansible-builder build --file 1534-raks.yml --tag 1534-raks:latest --container-runtime podman
```
...
- **1534-raks.yml**: This is your EE's "blueprint." It's the main definition file that ansible-builder will use to construct your image.
- **1534-raks-template.yml**: This is the Ansible self-service automation portal template file that generated this. You can import it and use it as a base to create new templates for your portal.
...
```bash
# This command uses your '1534-raks.yml' file to build an image
# and tags it as '1534-raks:latest'
...
```bash
ansible-navigator run playbook.yaml --eei 1534-raks:latest --pull-policy missing
```
...
podman tag 1534-raks:latest your-pah-hostname/1534-raks:latest

# Login to your Private Automation Hub
podman login your-pah-hostname

# Push the image
podman push your-pah-hostname/1534-raks:latest
```
...
podman tag 1534-raks:latest your-internal-registry.com/1534-raks:latest
podman login your-internal-registry.com
podman push your-internal-registry.com/1534-raks:latest
```
...
If you want to reuse this execution environment template for future projects, you can import the generated **1534-raks.yml** file into your self-service automation portal.
...
2. **Import from Git repository**: Enter the Git SCM URL containing your `1534-raks.yml` file, click **Analyze** to validate, review the details, then click **Import**.
